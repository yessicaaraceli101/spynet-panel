...
async function exportarPDF_Proveedor(proveedorNombre) {
  ...
  doc.setFontSize(22);
  doc.text(emp.nombre || "Mi Empresa", cx, 14, {
    align: "center"
  });
  ...
  if (emp.ruc) {
    doc.text(`RUC: ${emp.ruc}`, cx, 20, {
      align: "center"
    });
  }

  if (emp.direccion) {
    doc.text(emp.direccion, cx, emp.ruc ? 25 : 20, {
      align: "center"
    });
  }
  ...
  if (contacto) {
    doc.setFontSize(9);
    doc.text(contacto, cx, emp.direccion ? 30 : 25, {
      align: "center"
    });
  }
  ...
  pedidosProveedor.forEach(p => {
    ...
    items.forEach(i => {
      const subtotalTexto = (() => {
        ...
        if (subtotalItem > 0) {
          return "Gs. " + money(subtotalItem);
        }

        const totalCantPedido = items.reduce(
          (a, x) => a + Number(x.cantidad || 0),
          0
        ) || 0;
        ...
        if (subtotalPedido > 0 && totalCantPedido > 0) {
          const proporcional =
            subtotalPedido *
            (Number(i.cantidad || 0) / totalCantPedido);
  ...
  doc.text(
    `Generado automáticamente por ${emp.nombre || "Mi Empresa"}`,
    cx,
  ...
  const nombreArchivo =
    `Pedido_${limpiarNombreArchivo(emp.nombre)}_${limpiarNombreArchivo(proveedorNombre)}_${hoy}.pdf`;
...
function show(hash) {
  ...
  qsa(".sidebar-nav a[data-link]").forEach(a => {
    a.classList.toggle(
      "active",
      a.getAttribute("href") === targetHash
    );
  });
  ...
  if (targetHash === "#ventas") {
    if (typeof cargarVentas === "function") cargarVentas();
  }

  if (targetHash === "#clientes") {
    if (typeof listarClientes === "function") listarClientes();
  }

  if (targetHash === "#productos") {
    if (typeof listarProductos === "function") listarProductos();
  }

  if (targetHash === "#proveedores") {
    if (typeof listarProveedores === "function") listarProveedores();
  }

  if (targetHash === "#categorias") {
    if (typeof listarCategorias === "function") listarCategorias();
  }

  if (targetHash === "#compras") {
    if (typeof cargarComprasLista === "function") cargarComprasLista();
  }

  if (targetHash === "#lista_pedidos") {
    if (typeof listarPedidos === "function") listarPedidos();
  }

  if (targetHash === "#pedidos") {
    if (typeof cargarProveedoresPedido === "function") cargarProveedoresPedido();
  }

  if (targetHash === "#cuentas-pagar") {
  ...
  if (targetHash === "#formas-pago") {
    if (typeof listarFP === "function") listarFP();
  }
}
...
function agregarProductoAlPedido() {
  ...
  const cantidad = Number(
    document.getElementById("pp_edit_cantidad")?.value || 0
  );
  ...
  const costoTxt = (
    document.getElementById("pp_edit_costo")?.value || ""
  ).trim();
  ...
  if (typeof renderPP_Items === "function") {
    renderPP_Items();
  }
...
async function cargarProductosModalPP() {
  ...
  try {
    // Cargar productos e insumos en paralelo
    ...
    INSUMOS_CACHE = (insumos || []).map(i => ({
      ...
      marca: "",
      categoria: "Insumo",
      costo: Number(i.costo_promedio || 0),
      precio: Number(i.costo_promedio || 0),
      stock: Number(i.stock || 0),
      unidad: i.unidad || "Unidad",
      esInsumo: true
    }));

    // Inicializar listas para POS
    window.listaProductosPOS = [...PROD_CACHE];
    window.listaInsumosPOS = [...INSUMOS_CACHE];

    // Modo por defecto: productos
    window.modoSeleccion = 'productos';
    posFiltrados = [...window.listaProductosPOS];
    posPagina = 1;
    aplicarPaginaPOS();

    // Actualizar badge con el conteo de la pestaña activa
    actualizarBadge();

    // Marcar la pestaña activa visualmente
    const tabProd = document.getElementById("tabProductos");
    const tabIns = document.getElementById("tabInsumos");
    if (tabProd) {
      tabProd.classList.add("btn-primary");
      tabProd.classList.remove("btn-secondary");
    }
    if (tabIns) {
      tabIns.classList.remove("btn-primary");
      tabIns.classList.add("btn-secondary");
    }
...
function actualizarBadge() {
  ...
  const lista = window.modoSeleccion === 'insumos'
    ? window.listaInsumosPOS
    : window.listaProductosPOS;

  const total = lista.length;
  const label = window.modoSeleccion === 'insumos'
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (tabIns) {
    tabIns.addEventListener("click", () => {
      ...
      tabIns.classList.add("btn-primary");
      tabIns.classList.remove("btn-secondary");
      tabProd.classList.remove("btn-primary");
      tabProd.classList.add("btn-secondary");
...
async function jpost(url, data = {}) {
  ...
  if (!r.ok) {
    throw new Error(json.msg || json.error || "Error en el servidor");
  }
...
function renderPagination(containerSel, state, onChange) {
  ...
  el.innerHTML = `
    <button class="btn secondary" ${state.page <= 1 ? "disabled" : ""} data-act="prev">
      « Anterior
    </button>

    <span class="badge">
      Página ${state.page} / ${state.pages}
    </span>

    <button class="btn secondary" ${state.page >= state.pages ? "disabled" : ""} data-act="next">
      Siguiente »
    </button>
...
function exportTableCSV(tableSel, filename) {
  ...
  if (!table) {
    return alert("Tabla no encontrada");
  }

  let csv = [];

  for (const row of table.querySelectorAll("tr")) {
    let cols = [...row.children].map(td =>
  ...
  const empresaNombreLimpio =
    typeof limpiarNombreArchivo === "function"
      ? limpiarNombreArchivo(EMPRESA_NOMBRE || "empresa")
      : "empresa";
...
function printTableAsPDF(containerSel, title) {
  ...
  const empresaNombre =
    typeof EMPRESA_NOMBRE !== "undefined"
      ? EMPRESA_NOMBRE
      : "Mi Empresa";

  const empresaColor =
    typeof EMPRESA_COLOR !== "undefined"
      ? EMPRESA_COLOR
      : "#2563eb";
...
async function cargarKpis() {
  try {
    ...
    const compras = comprasRes.status === "fulfilled" && Array.isArray(comprasRes.value)
      ? comprasRes.value
      : [];

    const productos = productosRes.status === "fulfilled" && Array.isArray(productosRes.value)
      ? productosRes.value
      : [];

    const cuentas = cuentasRes.status === "fulfilled" && Array.isArray(cuentasRes.value)
      ? cuentasRes.value
      : [];
    ...
    const totalVenta = (v) =>
      Number(v.total_pyg ?? v.total ?? 0);

    const totalCompra = (c) =>
      Number(c.total_pyg ?? c.total ?? 0);

    const totalCuenta = (c) =>
      Number(c.monto_pyg ?? c.monto ?? c.total ?? 0);
    ...
    const cuentasPagadas = cuentas.filter(c =>
    ...
    const stockTotalUnidades = productos.reduce(
          ...
          .map(p => `
            <div class="stock-item">
              <div class="stock-item-left">
                ...
                <span>${escapeHtml(p.categoria || "Sin categoría")}</span>
              </div>
...
async function renderGraficoVentasCompras() {
  try {
    ...
    ventas.forEach(v => {
      ...
      ventasPorMes[mes] += Number(v.total_pyg ?? v.total ?? 0);
    });
    ...
    compras.forEach(c => {
      ...
      const mes = fecha.getMonth();
      comprasPorMes[mes] += Number(c.total_pyg ?? c.total ?? 0);
    });
        ...
        datasets: [
          {
            ...
            borderWidth: 1,
            borderRadius: 8,
            backgroundColor: EMPRESA_COLOR || "#2563eb"
          },
          {
            label: "Compras",
            data: comprasPorMes,
  ...
  pageData.forEach(cli => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align:center;">
        <button class="btn-icon edit" onclick="abrirEditarCliente(${Number(cli.id)})">
          <i class="fa fa-pen"></i>
        </button>

        <button class="btn-icon delete" onclick="eliminarCliente(${Number(cli.id)})">
...
function renderPaginacionClientes() {
  ...
  for (let i = 1; i <= total; i++) {
    div.innerHTML += `
      <button class="pag-btn ${i === clientesPaginaActual ? "active" : ""}"
        onclick="cambiarPaginaClientes(${i})">
        ${i}
      </button>
    `;
  }
  ...
  div.innerHTML += `
    <button class="pag-btn" onclick="cambiarPaginaClientes(${clientesPaginaActual + 1})"
      ${clientesPaginaActual === total ? "disabled" : ""}>
      ›
    </button>
  `;
}
...
function filtrarClientes(q = "") {
  ...
  clientesFiltrados = clientesOriginal.filter(c => {
    ...
    return (
      ...
      String(c.pais || "").toLowerCase().includes(q) ||
...
async function guardarCliente() {
  ...
  if (!body.ci) {
    return alert("El CI es obligatorio.");
  }
...
/* ============================================
      ...
      Compatible con /api/usuarios multiempresa
============================================ */
let USERS_CACHE = [];
let USERS_FILTER = [];
let USERS_PG = {
  ...
  const rows = pg.slice.map(u => {
    ...
    return `
      <tr>
        ...
        <td class="actions">
          <button class="btn secondary" onclick="abrirEditarUser(${Number(u.id)})">Editar</button>
          <button class="btn ghost" onclick="eliminarUser(${Number(u.id)})">Eliminar</button>
        </td>
      </tr>
    `;
  }).join("");
...
function renderPaginacionProveedores() {
  ...
  for (let i = 1; i <= total; i++) {
    div.innerHTML += `
      <button class="pag-btn ${i === provPaginaActual ? "active" : ""}"
        onclick="cambiarPaginaProveedores(${i})">
        ${i}
      </button>
    `;
  }
  ...
  div.innerHTML += `
    <button class="pag-btn" onclick="cambiarPaginaProveedores(${provPaginaActual + 1})"
      ${provPaginaActual === total ? "disabled" : ""}>
      ›
    </button>
  `;
}
  ...
  try {
    ...
    if (sel && Array.isArray(CAT_CACHE) && CAT_CACHE.length) {
      sel.innerHTML =
        ...
        CAT_CACHE
          .map(c => `<option value="${Number(c.id)}">${escapeHtml(c.nombre || "")}</option>`)
          .join("");
    }
...
function renderProductos(list) {
  ...
  const rows = productosPagina.map(p => {
    const catName =
      (CAT_CACHE.find(c => Number(c.id) === Number(p.categoria_id)) || {}).nombre ||
      p.categoria ||
      "-";
    ...
    return `
      <tr data-id="${Number(p.id)}">
        <td><input type="checkbox" aria-label="Seleccionar fila"></td>

        <td style="text-align:center;">${Number(p.id)}</td>
        ...
        <td style="display:flex; gap:.4rem;">
          <button class="btn-circle btn-edit" title="Editar" onclick="abrirEditar(${Number(p.id)})">
            <i class="fa fa-pen"></i>
          </button>

          <button class="btn-circle btn-del" title="Eliminar" onclick="eliminarProducto(${Number(p.id)})">
            <i class="fa fa-trash"></i>
          </button>
        </td>
      </tr>
...
function renderPaginacionProductos(list, per) {
  ...
  contenedor.innerHTML += `
    <button class="pag-btn" onclick="cambiarPaginaProductos(${productosPaginaActual - 1})"
      ${productosPaginaActual === 1 ? "disabled" : ""}>
      ‹
    </button>
  `;
  ...
  contenedor.innerHTML += `
    <button class="pag-btn" onclick="cambiarPaginaProductos(${productosPaginaActual + 1})"
      ${productosPaginaActual === totalPaginas ? "disabled" : ""}>
      ›
    </button>
  `;
}
...
function resetNuevoProducto() {
  ...
  const file = qs("#pr_img");
  if (file) file.value = "";

  const prev = qs("#pr_img_preview");
  if (prev) prev.src = "img/no-image.png";
}
...
function abrirEditar(id) {
  ...
  if (edCatSelect && p.categoria_id) {
    edCatSelect.value = String(p.categoria_id);
  }
...
async function actualizarProducto() {
  ...
  const body = {
    codigo: gv("#ed_codigo"),
    nombre: gv("#ed_nombre"),
    descripcion: gv("#ed_desc"),
    marca: gv("#ed_marca"),
    categoria_id,
    categoria,
    costo,
    precio,
    stock: Number(gv("#ed_stock") || 0)
  };
...
async function listarCategorias() {
  try {
    ...
    CAT_CACHE = (Array.isArray(data) ? data : []).map(c => ({
      ...c,
      id: Number(c.id),
      codigo: c.codigo || "",
      nombre: c.nombre || "",
    ...
    const opts = CAT_CACHE
      .map(c => `<option value="${Number(c.id)}">${escapeHtml(c.nombre || "")}</option>`)
      .join("");

    const prCat = qs("#pr_cat");
    if (prCat) {
      prCat.innerHTML = `<option value="">(sin categoría)</option>` + opts;
    }

    const edCat = qs("#ed_cat_select");
    if (edCat) {
      edCat.innerHTML = `<option value="">(sin categoría)</option>` + opts;
    }
  ...
  const rows = categoriasPagina.map(c => {
    ...
    const imgHtml = imgSrc
      ? `
        <img src="${escapeHtml(imgSrc)}"
             alt="${escapeHtml(c.nombre || "Categoría")}"
             onerror="this.src='img/no-image.png'"
    ...
    return `
      <tr data-id="${Number(c.id)}">
        <td class="text-center align-middle">${Number(c.id)}</td>
        <td class="text-center align-middle">${imgHtml}</td>
        <td class="align-middle">${escapeHtml((c.codigo ?? "").toString().trim() || "—")}</td>
        <td class="align-middle">${escapeHtml(c.nombre || "—")}</td>
        <td class="align-middle">${escapeHtml(c.descripcion || "—")}</td>
        <td class="text-center align-middle">
          <div class="btn-group">
            <button class="btn btn-sm btn-light-primary me-1"
              title="Editar"
              onclick="abrirEditarCategoria(${Number(c.id)})"
              style="background-color:#e0e7ff;color:${EMPRESA_COLOR || "#2563eb"};border-radius:50%;width:40px;height:40px;display:inline-flex;align-items:center;justify-content:center;">
              <i class="fa-solid fa-pen"></i>
            </button>

            <button class="btn btn-sm btn-light-danger"
              title="Eliminar"
              onclick="eliminarCategoria(${Number(c.id)})"
  ...
  cont.innerHTML = `
    <table class="table align-middle">
      ...
      <tbody>
        ${
          rows ||
          '<tr><td colspan="6" class="text-center text-muted py-3">Sin categorías para esta empresa</td></tr>'
        }
      </tbody>
    </table>
  `;
...
function renderPaginacionCategorias(list) {
  ...
  div.innerHTML += `
    <button class="pag-btn" onclick="cambiarPaginaCategorias(${categoriasPaginaActual - 1})"
      ${categoriasPaginaActual === 1 ? "disabled" : ""}>
      ‹
    </button>
  `;
  ...
  div.innerHTML += `
    <button class="pag-btn" onclick="cambiarPaginaCategorias(${categoriasPaginaActual + 1})"
      ${categoriasPaginaActual === total ? "disabled" : ""}>
      ›
    </button>
  `;
}
...
async function guardarCategoria() {
  try {
    ...
    const body = {
      nombre: gv("#cat_nombre"),
      codigo: gv("#cat_codigo"),
      descripcion: gv("#cat_desc"),
      imagen_base64: img || null
    };
...
async function actualizarCategoria() {
  try {
    ...
    const body = {
      nombre: gv("#cat_ed_nombre"),
      codigo: gv("#cat_ed_codigo"),
      descripcion: gv("#cat_ed_desc")
    };
  ...
  try {
    ...
    const rows = compras.map(c => `
      <tr>
        ...
        <td>${escapeHtml(c.productos || c.nombres_productos || "-")}</td>
        <td>Gs. ${money(c.total_pyg ?? c.total ?? 0)}</td>
        <td>${escapeHtml(c.proveedor_nombre || "-")}</td>
        <td>${escapeHtml(c.proveedor_ruc || "-")}</td>
      </tr>
    `).join("");
    ...
    tabla.innerHTML = `
      <table class="table">
        ...
        <tbody>
          ${rows || `<tr><td colspan="6" style="text-align:center;padding:1rem;">Sin compras para esta empresa</td></tr>`}
        </tbody>
      </table>
    `;
...
async function renderDashboard() {
  try {
    ...
    if (kpiVentas) {
      kpiVentas.innerHTML =
        `Año: Gs. ${money(totV.totalAnual)}<br>
         Mes: Gs. ${money(totV.totalMensual)}`;
    }

    if (kpiCompras) {
      kpiCompras.innerHTML =
        `Año: Gs. ${money(totC.totalAnual)}<br>
         Mes: Gs. ${money(totC.totalMensual)}`;
    }

    if (kpiProductos) {
      kpiProductos.innerText = stockTotal;
    }
...
async function marcarComoRecibido(id) {
  return recibirPedido(id);
}

async function exportarPDF_Listado() {
  const emp = typeof getEmpresaActualPDF === "function"
    ? getEmpresaActualPDF()
    : {
        nombre: EMPRESA_NOMBRE || "Mi Empresa",
        logo: EMPRESA_LOGO || "img/logo2.png",
        color: EMPRESA_COLOR || "#2563eb",
  ...
  const colorPrincipal =
    typeof hexToRgbArray === "function"
      ? hexToRgbArray(emp.color)
      : [37, 99, 235];

  try {
    if (logoConsorcio) {
      doc.addImage(logoConsorcio, imgType(logoConsorcio), 10, 8, 32, 18);
    }

    if (logoSpynet) {
      doc.addImage(logoSpynet, imgType(logoSpynet), pageW - 10 - 28, 8, 28, 18);
    }
  } catch (e) {
    console.warn("Error addImage logos:", e);
  }
  ...
  doc.setFont("times", "bold");
  doc.setFontSize(22);
  doc.text(emp.nombre || "Mi Empresa", cx, 14, { align: "center" });
  ...
  if (contacto) {
    doc.setFontSize(9);
    doc.text(contacto, cx, emp.direccion ? 30 : 25, { align: "center" });
  }
  ...
  const rows = (pedidos || []).map(p => {
    const productos = p.items?.length
      ? p.items.map(i => i.producto_nombre || "—").join(", ")
      : "—";

    const categorias = p.items?.length
      ? p.items.map(i => i.categoria_nombre || "—").join(", ")
      : "—";

    const cantidad_items =
      p.items?.reduce((a, i) => a + Number(i.cantidad || 0), 0) || 0;
  ...
  doc.autoTable({
    ...
    theme: "striped",
    head: [[
      "ID", "Proveedor", "Productos", "Categorías", "Fecha Pedido",
...
async function exportarPDF_Pedido() {
  ...
  try {
    p = await jget(`/api/pedidos/${id}`);
  } catch (err) {
    console.error("Error cargando pedido:", err);
    return alert("No se pudo cargar el pedido.");
  }
  ...
  try {
    ...
    if (logoSpynet) doc.addImage(logoSpynet, imgType(logoSpynet), pageW - 38, 8, 28, 18);
  } catch (e) {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(14);
  doc.text(`Pedido a Proveedor #${p.id}`, 14, 52);
...
async function cargarProveedoresEnSelect() {
  ...
  try {
    ...
    if (!Array.isArray(proveedores) || !proveedores.length) {
      select.innerHTML = `<option value="">No hay proveedores para esta empresa</option>`;
      return;
    }
...
async function listarFP() {
  ...
  if (!keys.length) {
    cont.innerHTML = `
      <div class="text-muted" style="padding:.75rem;">
        Sin ventas pagadas para esta fecha.
      </div>
    `;
  } else {
    cont.innerHTML = `
...
function renderMovimientosComprobante() {
  ...
  tbodyMov.innerHTML = pagina.map(v => `
    <tr style="border-top:1px solid #e5e7eb;">
      ...
      <td style="padding:10px;">
        ${escapeHtml(normalizarFormaPago(v))}
      </td>

      <td style="padding:10px;">
        <b>${escapeHtml(v.nro_comprobante || "—")}</b>
      </td>

      <td style="padding:10px; text-align:right;">
        Gs. ${money(v.total_pyg ?? v.total ?? 0)}
      </td>
    </tr>
  `).join("");
...
function renderPaginacionMovimientosComprobante() {
  ...
  div.innerHTML += `
    <button
      class="pag-btn"
      onclick="cambiarPaginaMovComprobante(${fpMovPaginaActual - 1})"
      ${fpMovPaginaActual === 1 ? "disabled" : ""}
    >
      ‹
    </button>
  `;
  ...
  for (let i = 1; i <= total; i++) {
    div.innerHTML += `
      <button
        class="pag-btn ${i === fpMovPaginaActual ? "active" : ""}"
        onclick="cambiarPaginaMovComprobante(${i})"
      >
        ${i}
      </button>
    `;
  ...
  div.innerHTML += `
    <button
      class="pag-btn"
      onclick="cambiarPaginaMovComprobante(${fpMovPaginaActual + 1})"
      ${fpMovPaginaActual === total ? "disabled" : ""}
    >
      ›
    </button>
  `;
...
async function crearFP() {
  ...
  try {
    ...
    console.error(err);
    alert("No se pudo crear la forma de pago.");
  }
}
...
function updateCantidadCompra(i, val) {
  ...
  compraItems[i].subtotal =
    compraItems[i].cantidad * compraItems[i].costo;
...
function recalcularCompra() {
  subtotalCompra = compraItems.reduce(
    (a, i) => a + Number(i.subtotal || 0),
    0
  );
...
async function guardarCompra() {
  if (!compraItems.length) {
    return alert("No hay productos en la compra.");
  }

  const proveedorId = Number(
    document.getElementById("c_proveedor")?.value || 0
  );
  ...
  const compra = {
    ...
    fecha: document.getElementById("c_fecha")?.value || null,
    ...
    items: compraItems.map(i => ({
      ...
      subtotal: Number(i.cantidad) * Number(i.costo)
    }))
  };
...
function seleccionarProductoCompra(prod) {
  ...
  document.getElementById("c_producto_id").value =
    prod.id || "";
...
async function cargarProductosPedido(buscar = "") {
  try {
    const productos = await jget(
      `/productos${buscar ? `?buscar=${encodeURIComponent(buscar)}` : ""}`
    );
    ...
    productos.forEach(p => {
      ...
      tr.innerHTML = `
        ...
        <td>${escapeHtml(p.marca || "-")}</td>
        <td>${escapeHtml(p.categoria || "-")}</td>
        <td>Gs. ${money(p.costo || 0)}</td>
        <td>${Number(p.stock || 0)}</td>
        <td>
          <button class="btn btn-primary" onclick="seleccionarProductoPP(${Number(p.id)})">
            Seleccionar
          </button>
        </td>
      `;
...
async function cargarProveedoresPedido() {
  ...
  try {
    ...
    if (!Array.isArray(data) || !data.length) {
      sel.innerHTML = `<option value="">No hay proveedores para esta empresa</option>`;
      return;
    }
...
function editarPP_Campo(i, campo, valor) {
  ...
  if (campo === 'cantidad') {
    ...
    // Recalcular cantidad_base según la unidad elegida
  ...
  // Recalcular subtotal usando la cantidad convertida
...
function calcularTotalesPP() {
  ...
  // Sumar los subtotales ya calculados (en lugar de recalcular)
  const subtotal = items.reduce((acc, x) => acc + (x.subtotal || 0), 0);

  const iva = 0; // IVA desactivado
  const total = subtotal + iva;

  document.querySelectorAll("#pp_subtotal").forEach(el => {
    el.textContent = fmtPYG(subtotal);
  });

  document.querySelectorAll("#pp_iva").forEach(el => {
    el.textContent = fmtPYG(iva);
  });

  document.querySelectorAll("#pp_total").forEach(el => {
    el.textContent = fmtPYG(total);
  });
}
...
function seleccionarProductoPP(id) {
  // Buscar en la lista activa (según modo)
  let p;
  ...
  // Mostrar/ocultar controles de insumo
  ...
  if (esInsumo) {
    // Mostrar controles de insumo, ocultar campo de unidad de producto
    insumoControls.style.display = "block";
    if (campoUnidadProducto) campoUnidadProducto.style.display = "none";

    // Seleccionar la unidad del insumo en el select
    const unidadSelect = document.getElementById("pp_edit_unidad_select");
    if (unidadSelect) {
      if (p.unidad) unidadSelect.value = p.unidad;
      else unidadSelect.value = "Kg";
    }
  } else {
    // Ocultar controles de insumo, mostrar campo de unidad de producto
    insumoControls.style.display = "none";
    if (campoUnidadProducto) campoUnidadProducto.style.display = "block";

    // Si hay un input de unidad para productos, lo dejamos como estaba
...
function setCantidadInsumo(factor) {
  ...
  let cantidadBase = 1;
  // Si la unidad es g o ml, interpretamos "1" como 1000 (1 Kg o 1 L)
...
function renderizarProductosPOS(productos) {
  ...
  grid.innerHTML = productos.map(p => {
    ...
    // Mostrar SKU o código
    const sku = p.codigo || p.id || "";
    const nombre = p.nombre || "";
    // Para insumos, mostramos su unidad en lugar de marca
    const detalle = esInsumo ? (p.unidad || "Unidad") : (p.marca || "");
    // Precio: para insumos usamos costo_promedio, para productos precio
    const precio = esInsumo ? (p.costo || 0) : (p.precio || 0);

    return `
      <div class="card-producto-pos">
        <div class="card-top-pos">
          <span class="card-sku-pos">${sku}</span>
          <span class="stock-badge-pos ${p.stock > 0 ? 'stock-ok-pos' : 'stock-no-pos'}">
            ${p.stock > 0 ? `Stock: ${p.stock}` : "Sin stock"}
          </span>
        </div>
        <div class="card-nombre-pos">${nombre}</div>
        <div class="card-marca-pos">${detalle}</div>
        <!-- CAMBIO IMPORTANTE: uso money() para formatear con separadores de miles -->
        <div class="card-precio-pos">${money(precio)}</div>
...
function renderizarPaginacionPOS(total) {
  ...
  if (info) {
    ...
    info.textContent =
      `Mostrando ${desde + 1}–${Math.min(desde + POS_POR_PAGINA, total)} de ${total} ${tipo}`;
  }
...
function renderPP_Items() {
  ...
  items.forEach((it, i) => {
    tbody.innerHTML += `
      <tr>
        <td>${escapeHtml(it.nombre || it.producto_nombre || "-")}</td>
        <td>
          <input type="number" min="1" step="any" value="${Number(it.cantidad || 0)}"
            onchange="editarPP_Campo(${i}, 'cantidad', this.value)" class="pp-cell">
        </td>
        <td>
          <input type="text" value="${money(it.costo || 0)}"
            onchange="editarPP_Campo(${i}, 'costo', this.value)" class="pp-cell">
        </td>
        <td>${fmtPYG(it.subtotal || 0)}</td>
        <td>
          <button class="pp-del" onclick="eliminarPP_Item(${i})">
            <i class="fa fa-trash"></i>
          </button>
        </td>
      </tr>
...
function filtrarProductosModalPP() {
  ...
  let lista;
  if (window.modoSeleccion === "insumos") {
    lista = window.listaInsumosPOS || [];
  } else {
    lista = window.listaProductosPOS || [];
  }
  posFiltrados = lista.filter(p =>
  ...
  (lista || []).forEach(p => {
    sel.innerHTML += `
      <option value="${Number(p.id)}">
        ${escapeHtml(p.nombre || "-")} — ${escapeHtml(p.marca || "")} (${escapeHtml(p.codigo || "Sin código")})
      </option>
    `;
  });
}
...
function guardarProductoAListaPP() {
  ...
  // Convertir cantidad a la unidad base
  const unidadBase = unidadBaseDe(unidad);
  const factor = obtenerFactorConversion(unidad, unidadBase);
  ...
  const item = {
    ...
    cantidad: cantidad,        // cantidad tal cual la ingresó (ej: 500)
    unidad: unidad,            // unidad elegida (ej: "g")
    cantidad_base: cantidadBase, // cantidad convertida (ej: 0.5 Kg)
    unidad_base: unidadBase,
    costo: costo,              // precio por unidad base (ej: 6000 Gs/Kg)
    subtotal: subtotal,
    total: subtotal,
    esInsumo: esInsumo
  };
...
function cargarSelectModalPP() {
  ...
  sel.innerHTML =
    ...
    (PROD_CACHE || []).map(p =>
      `<option value="${Number(p.id)}">
        ${escapeHtml(p.nombre || "-")} — ${escapeHtml(p.marca || "")} (${escapeHtml(p.codigo || "-")})
      </option>`
    ).join("");
}
...
async function cargarDashboardKpis() {
  try {
    ...
    const ventasHoy = ventas.filter(v =>
      toYMD(v.fecha || v.created_at) === hoyYMD
    );
    ...
    console.error("Error cargando KPIs dashboard:", err);
  }
}
...
async function cargarFormasPago() {
  try {
    ...
    select.innerHTML = (formas || [])
      .map(f => `<option value="${Number(f.id)}">${escapeHtml(f.nombre || "")}</option>`)
      .join("");
...
async function abrirCaja(tipoParam) {
  try {
    ...
    if (id) {
      ...
      if (!window.cajasActuales[tipo]?.id) {
...
function pintarEstadoCaja(idElemento, label, caja, saldoNetoGs) {
  ...
  // Si nos pasaron el saldo neto (ingreso - egreso) del resumen del día,
  // usamos ese en vez de caja.saldo_actual_gs, que solo trae el ingreso
  // acumulado y no resta los egresos (compras de insumos, etc.)
  const saldoGs = saldoNetoGs != null
    ? Number(saldoNetoGs)
    : Number(caja.saldo_actual_gs ?? caja.saldo_gs ?? caja.saldo_inicial ?? 0);
...
async function cerrarCaja(tipoParam) {
  try {
    ...
    try {
      data = await jpost(`/caja/cerrar/${id}`, {});
    } catch {
      data = await jpost("/caja/cerrar", { caja_id: id });
    }
...
async function cargarFormasPagoResumen() {
  try {
    ...
    ventas.forEach(v => {
      ...
      const esEfectivo =
        metodo.includes("efect") ||
        Number(v.forma_pago_id) === 2;

      const esTransfer =
        metodo.includes("transf") ||
        Number(v.forma_pago_id) === 3;
...
async function verificarCaja() {
  try {
    ...
    // Traemos también el resumen del día (ya trae ingreso-egreso neto)
    ...
    // Resumen total (usa el mismo saldo neto ya correcto)
...
(function bindCajaEventos() {
  const bind = (el) => {
    ...
    el.addEventListener("change", async () => {
      ...
      if (location.hash === "#formas-pago" && typeof listarFP === "function") {
        listarFP();
      }
    });
  };
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="7" style="text-align:center">
          No hay cuentas a pagar para esta empresa.
        </td>
      </tr>
    `;
    return;
  ...
  tbody.innerHTML = data.map(item => {
    ...
    return `
      <tr>
        ...
        <td>${item.vencimiento ? toYMD(item.vencimiento) : "-"}</td>
        ...
        <td>
          <div class="acciones-tabla">
            ${
              estado !== "pagado"
                ? `<button class="btn btn-sm btn-success" onclick="pagarCuentaPagar(${Number(item.id)})">Pagar</button>`
                : ""
            }

            <button class="btn-icon edit" title="Editar" onclick="editarCuentaPagar(${Number(item.id)})">
              <i class="fa fa-pen"></i>
            </button>

            <button class="btn-icon delete" title="Eliminar" onclick="eliminarCuentaPagar(${Number(item.id)})">
              <i class="fa fa-trash"></i>
            </button>
          </div>
          ...
          ${
            estado === "pagado"
              ? `<div style="margin-top:.35rem; font-size:.82rem; color:#6b7280;">
                  Pago: ${escapeHtml(fechaPago)} | Caja: ${escapeHtml(tipoCaja)}
                 </div>`
              : ""
          }
        </td>
...
async function guardarCuentaPagar() {
  ...
  if (!proveedor || !concepto || !montoPyg) {
    return alert("Complete proveedor, concepto y monto.");
  }
...
async function descargarInformeCajaPDF() {
  ...
  function sameMonthPDF(fecha, ref) {
  ...
  const cuentasMes = cuentas.filter(c => {
    const estado = String(c.estado || "").toLowerCase();
    const fechaPago = onlyDate(c.fecha_pago || c.pagado_en || c.fecha || "");
    return estado === "pagado" && sameMonthPDF(fechaPago, hoy);
  });

  const resumenDia = {
    ventas: ventasDia.reduce((a, b) => a + Number(b.total_pyg ?? b.total ?? 0), 0),
    compras: comprasDia.reduce((a, b) => a + Number(b.total_pyg ?? b.total ?? 0), 0),
    cuentas: cuentasDia.reduce((a, b) => a + Number(b.monto_pyg ?? b.monto ?? 0), 0)
  };

  const resumenMes = {
    ventas: ventasMes.reduce((a, b) => a + Number(b.total_pyg ?? b.total ?? 0), 0),
    compras: comprasMes.reduce((a, b) => a + Number(b.total_pyg ?? b.total ?? 0), 0),
    cuentas: cuentasMes.reduce((a, b) => a + Number(b.monto_pyg ?? b.monto ?? 0), 0)
  };

  async function cargarImagen(url) {
    try {
      return await cargarLogoBase64(url);
    } catch {
      return null;
    }
  }
  ...
  function dibujarEncabezado(yBase = 10) {
    if (logo) {
      try {
        doc.addImage(logo, imgType(logo), 10, yBase, 30, 22);
        doc.addImage(logo, imgType(logo), 170, yBase, 28, 22);
      } catch {}
    }

    doc.setFont("helvetica", "bold");
    doc.setFontSize(18);
    doc.text(emp.nombre || "Mi Empresa", 105, yBase + 10, { align: "center" });
    ...
    doc.setDrawColor(0);
    doc.line(10, yBase + 38, 200, yBase + 38);
  }

  dibujarEncabezado(8);

  doc.setFontSize(16);
  doc.text("Informe de Caja", 14, 58);

  doc.setFontSize(10);
  doc.text(`Fecha de emisión: ${fechaHora}`, 14, 65);
  doc.text(`Generado por: ${usuario}`, 14, 71);
  ...
  doc.setFontSize(14);
  doc.text("Detalle de Ventas del Día", 14, 58);
  ...
  doc.autoTable({
    ...
    body: ventasDia.length
      ? ventasDia.map(v => [
          onlyDate(v.fecha || v.created_at),
          v.cliente_nombre || "Consumidor Final",
          v.forma_pago_nombre || "-",
          v.productos || "-",
          fmtGs(v.total_pyg ?? v.total ?? 0)
        ])
  ...
  doc.setFontSize(14);
  doc.text("Detalle de Ventas del Mes", 14, 58);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `Documento generado automáticamente • Empresa: ${emp.nombre || "Mi Empresa"} • Usuario: ${usuario} • Página ${i}/${totalPages}`,
      105,
      290,
      { align: "center" }
    );
...
async function toggleMonedaCuentaPagar() {
  ...
  if (moneda === "PYG") {
    ...
    try {
      ...
      tipoCambio = Number(encontrada?.tipo_cambio || encontrada?.valor || 1);
...
function actualizarResumenCuentaPagar() {
  ...
  const montoConvertido =
    moneda === "USD" || moneda === "BRL"
      ? tipoCambio > 0 ? montoGs / tipoCambio : 0
      : montoGs;
  ...
  if (elMoneda) {
    elMoneda.textContent = fmtMonedaCuenta(moneda, montoConvertido);
  }
}
  ...
  try {
      ...
      tbody.innerHTML = `
        <tr>
          <td colspan="7" style="text-align:center;padding:12px;">
            No hay ventas registradas para esta empresa.
          </td>
        </tr>
      `;
      return;
    ...
    tbody.innerHTML = ventas.map(v => `
      <tr>
        <td>${v.id}</td>
        <td>${toYMD(v.fecha)}</td>
        <td>${escapeHtml(v.cliente_nombre || "Consumidor Final")}</td>
        <td>${escapeHtml(v.productos || "-")}</td>
        <td>${escapeHtml(v.forma_pago_nombre || "-")}</td>
        <td>Gs. ${money(v.total_pyg ?? v.total ?? 0)}</td>
        <td>${escapeHtml(v.estado_pago || "-")}</td>
      </tr>
    `).join("");
...
let insumos = [];        // caché local de insumos (se recarga desde API)
  ...
  try {
        ...
        <tr>
          <td colspan="6" style="text-align:center;padding:30px;color:#64748b">
            <div class="empty-state" style="border:none;background:transparent;padding:0">
              ...
              <strong>No hay insumos cargados todavía</strong>
              <p>Hacé clic en "Agregar Insumo" para crear el primero.</p>
            </div>
          </td>
        </tr>`;
    ...
    insumos.forEach(i => {
      ...
      tr.innerHTML = `
        ...
        <td>${i.nombre}</td>
        <td>${i.unidad}</td>
        <td class="${bajoMinimo ? 'stock-low' : 'stock-ok'}">${(i.stock ?? 0)} ${i.unidad}</td>
        <td>${(i.stock_min ?? 0)} ${i.unidad}</td>
        <td>
          <button class="action-icon-btn edit" onclick="editarInsumo(${i.id})" title="Editar">
            <svg class="icon icon-sm"><use href="#icon-edit"/></svg>
          </button>
          <button class="action-icon-btn delete" onclick="eliminarInsumo(${i.id})" title="Eliminar">
...
function abrirNuevoInsumo() {
  ...
  document.getElementById("insumo_titulo").innerHTML =
    `<svg class="icon"><use href="#icon-box"/></svg> Nuevo Insumo`;
  renderStockRapido();
...
function renderStockRapido() {
  ...
  const opciones = [
    { label: "¼", valor: base * 0.25 },
    { label: "½", valor: base * 0.5 },
    { label: "1", valor: base * 1 },
    { label: "5", valor: base * 5 },
  ];
  ...
  opciones.forEach(op => {
    ...
    btn.onclick = () => {
      document.getElementById("insumo_stock").value = op.valor;
    };
...
async function editarInsumo(id) {
  ...
  document.getElementById("insumo_titulo").innerHTML =
    `<svg class="icon"><use href="#icon-edit"/></svg> Editar Insumo`;
  renderStockRapido();
  openModal("modalInsumo");
}
...
async function guardarInsumo() {
  ...
  try {
    if (id) {
      await jput("/insumos/" + id, body);
      toast("Insumo actualizado", "success");
    } else {
      await jpost("/insumos", body);
    ...
    const msg = err.detalle || err.message || "Error al guardar insumo";
    toast(msg, "error");
  }
}
...
async function confirmarEliminarInsumo() {
  ...
  try {
    ...
    toast("Error al eliminar insumo", "error");
  }
...
async function abrirCompraInsumo() {
  try {
    ...
    console.error("Error al abrir compra de insumo:", err);
...
async function guardarCompraInsumo() {
  ...
  if (!insumo_id) {
    ...
    return toast("Seleccioná un insumo.", "error");
  }

  const cantidad = parseFloat(document.getElementById("ci_cantidad").value) || 0;
  if (cantidad <= 0) {
    if (btn) btn.disabled = false;
    return toast("Cantidad debe ser mayor a 0.", "error");
  }
  ...
  if (!insumo) {
    if (btn) btn.disabled = false;
    return toast("Insumo no encontrado.", "error");
  }
  ...
  if (!proveedor_id) {
    if (btn) btn.disabled = false;
    return toast("Seleccioná un proveedor.", "error");
  }
  ...
  try {
    ...
    try {
      ...
      await jpost("/compras", {
        ...
        items: [{
          ...
          cantidad: cantidadBase,
          costo: precioUnitarioBase,
          subtotal: precioTotal
        }]
      });
    } catch (err) {
      console.warn("No se pudo registrar la compra en egresos:", err);
    }
    ...
    console.error("Error guardando compra de insumo:", err);
    const msg = err.detalle || err.message || "Error al registrar compra";
    toast(msg, "error");
  } finally {
    if (btn) {
      btn.disabled = false;
...
async function abrirUsoInsumo() {
  try {
    ...
    insumosApi.forEach(i => {
      const opt = document.createElement("option");
      opt.value = i.id;
      opt.textContent = `${i.nombre} (stock: ${i.stock ?? 0} ${i.unidad})`;
      sel.appendChild(opt);
    });
    ...
    console.error("Error al abrir uso de insumo:", err);
    toast("Error al cargar datos", "error");
  }
}
...
async function guardarUsoInsumo() {
  ...
  try {
    ...
    console.error("Error guardando uso de insumo:", err);
...
function fechaISOaDate(fechaStr) {
  const [y, m, d] = fechaStr.split("-").map(Number);
  return new Date(y, m - 1, d);
}

function nombreDia(fechaStr) {
  return DIAS_SEMANA[fechaISOaDate(fechaStr).getDay()];
}
...
async function renderResumenInsumos(resetPaginas = true) {
  ...
  try {
    // Obtener todos los datos y filtrar en frontend (más simple)
    const [usosApi, comprasApi] = await Promise.all([
      jget("/usos-insumo") || [],
      jget("/compras-insumo") || []
    ]);
    ...
    const usosConCosto = usosEnRango.map(u => {
      ...
      return {
        ...u,
        nombreInsumo: insumo ? insumo.nombre : `#${u.insumo_id}`,
        costo: u.costo || 0
      };
    });
    ...
    usosEnRango.forEach(u => {
      totalesPorInsumo[u.insumo_id] = (totalesPorInsumo[u.insumo_id] || 0) + u.cantidad;
    });
    let insumoTopNombre = "—";
    let maxCantidad = 0;
    Object.entries(totalesPorInsumo).forEach(([id, cant]) => {
      if (cant > maxCantidad) {
        ...
        insumoTopNombre = insumo ? `${insumo.nombre} (${cant.toFixed(2)} ${insumo.unidad})` : `#${id}`;
      }
    });
    ...
    tbodyDias.innerHTML = paginaDias.length
      ? paginaDias.map(f => `
          <tr style="${f.tieneUsos ? "" : "color:#94a3b8"}">
            <td>${f.dia}</td>
            <td>${f.fecha.split("-").reverse().join("/")}</td>
            <td>${f.tieneUsos ? f.cantidadTotal.toFixed(2) : "—"}</td>
            <td>${f.tieneUsos ? formatGs(f.gastoTotal) : "Sin consumo"}</td>
          </tr>`).join("")
      : `<tr><td colspan="4" style="text-align:center;color:#94a3b8">Sin datos en este rango</td></tr>`;

    renderPaginacion("resumen-dias-paginacion", filasDias.length, RESUMEN_DIAS_POR_PAGINA, resumenDiasPagina, (nuevaPagina) => {
      resumenDiasPagina = nuevaPagina;
      renderResumenInsumos(false);
    });
    ...
    tbodyDetalle.innerHTML = paginaDetalle.length
      ? paginaDetalle.map(u => `
          <tr>
            <td>${u.fecha.split("-").reverse().join("/")}</td>
            <td>${nombreDia(u.fecha)}</td>
            <td>${u.nombreInsumo}</td>
            <td>${u.cantidad.toFixed(2)} ${u.unidad}</td>
            <td>${formatGs(u.costo)}</td>
            <td>${u.motivo || "—"}</td>
          </tr>`).join("")
      : `<tr><td colspan="6" style="text-align:center;color:#94a3b8">Sin usos registrados en este rango</td></tr>`;

    renderPaginacion("resumen-detalle-paginacion", detalleOrdenado.length, RESUMEN_DETALLE_POR_PAGINA, resumenDetallePagina, (nuevaPagina) => {
      resumenDetallePagina = nuevaPagina;
      renderResumenInsumos(false);
    });

  } catch (err) {
    console.error("Error renderizando resumen de insumos:", err);
    toast("Error al cargar resumen", "error");
  }
}

/* ============================================
   PROVEEDORES PARA COMPRA DE INSUMO (ya usa API)
   ============================================ */
...
async function cargarProveedoresEnSelectCompraInsumo() {
  ...
  try {
    ...
    // Sincronizar RUC visible y oculto
...
function renderHistorialCompras() {
  ...
  let datosFiltrados = historialCompras.filter(c =>
    ...
    (c.proveedor_ruc || "").toLowerCase().includes(filtro) // ← también buscar por RUC
  );
  ...
  if (!pagina.length) {
    ...
    tbody.innerHTML = pagina.map(c => `
      <tr>
        <td>${c.id}</td>
        <td>${c.fecha || "-"}</td>
        <td><strong>${c.proveedor || "Sin proveedor"}</strong></td>
        <td>${c.proveedor_ruc || "-"}</td>   <!-- ← CAMBIADO: RUC en lugar de factura -->
        <td>${c.insumo_nombre || `#${c.insumo_id}`}</td>
        <td>${c.cantidad_compra || c.cantidad} ${c.unidad_compra || c.unidad || ""}</td>
        <td>${c.unidad || "-"}</td>
        <td>Gs. ${Number(c.precio_unitario_base || 0).toLocaleString("es-PY")}</td>
        <td><strong>Gs. ${Number(c.precio_total || 0).toLocaleString("es-PY")}</strong></td>
      </tr>
    `).join("");
  }
  ...
  if (paginacion) {
    ...
    if (totalPaginas > 1) {
      paginacion.innerHTML = `
        <button ${historialPagina === 1 ? "disabled" : ""} onclick="cambiarPaginaHistorial(${historialPagina - 1})">‹ Anterior</button>
        <span>Página ${historialPagina} de ${totalPaginas}</span>
        <button ${historialPagina === totalPaginas ? "disabled" : ""} onclick="cambiarPaginaHistorial(${historialPagina + 1})">Siguiente ›</button>
      `;
    }
  }
}
...
function renderCompraRapido() {
  ...
  opciones.forEach(op => {
    ...
    btn.onclick = () => {
      document.getElementById("ci_cantidad").value = op.valor;
      calcularTotalCompraInsumo();
    };
    cont.appendChild(btn);
  });
...
function renderUsoRapido(unidad) {
  ...
  opciones.forEach(op => {
    ...
    btn.textContent = `${op.label} ${unidadMostrar}`;
    btn.onclick = () => {
      document.getElementById("uso_cantidad").value = op.valor;
      calcularCostoUso();
    };
    cont.appendChild(btn);
  });
...
function actualizarUnidadCompraInsumo() {
  ...
  if (unidadBase && select.querySelector(`option[value="${unidadBase}"]`)) {
    select.value = unidadBase;
  } else {
    select.value = "g";
  }
...
function actualizarUnidadUso() {
  ...
  document.getElementById("uso_stock_actual").textContent = insumo
    ? `Stock actual: ${insumo.stock ?? 0} ${insumo.unidad}`
    : "";
...
// Calcular costo estimado en uso
function calcularCostoUso() {
  ...
  if (costoPromedio === 0) {
    // Si no tiene costo, intentamos calcular desde compras guardadas
    // pero como ahora es API, mejor mostrar mensaje
    if (refEl) {
      refEl.innerHTML = `⚠️ "${insumo.nombre}" no tiene costo cargado. Editalo y completá "Costo de referencia", o registrá una Compra para este insumo.`;
      refEl.style.color = "#dc2626";
    }
  } else {
    const factor = obtenerFactorConversion(unidadUso, insumo.unidad);
    const cantidadBase = cantidadUso * factor;
    ...
    document.getElementById("uso_costo_estimado").textContent =
      "Gs. " + Math.round(costoEstimado).toLocaleString("es-PY");

    if (refEl) {
      refEl.textContent = `Calculado con costo promedio: Gs. ${Math.round(costoPromedio).toLocaleString("es-PY")} por ${insumo.unidad}`;
...
// DOMContentLoaded para cargar insumos si existe la tabla
document.addEventListener("DOMContentLoaded", () => {
