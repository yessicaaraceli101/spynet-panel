...
async function enviarPedidoProveedor({
  ...
  if (!proveedorEmail) {
    console.log("Proveedor sin email");
    return;
  }
  ...
  const transporter = nodemailer.createTransport({
    ...
    port: Number(empresa.smtp_port || 465),
    secure: !!empresa.smtp_secure,
    auth: {
  ...
  await transporter.sendMail({
    from: `"${empresaNombre}" <${empresa.smtp_from}>`,
    to: proveedorEmail,
  ...
  console.log("Pedido enviado a:", proveedorEmail);
}
...
function normTipoCaja(t) {
  ...
  if (s.includes("trans")) s = "transferencia";
  if (s.includes("efect")) s = "efectivo";
  return s;
}
...
app.put("/categorias/:id", requireEmpresa, async (req, res) => {
  try {
    ...
    if (!rows.length) {
      return res.status(404).json({ error: "Categoría no encontrada o no pertenece a esta empresa" });
    }
    res.json(rows[0]);
  } catch (e) {
...
app.delete("/productos/:id", requireEmpresa, async (req, res) => {
  try {
    ...
    if (!rowCount) {
      return res.status(404).json({ ok: false, msg: "Producto no encontrado o no pertenece a esta empresa" });
    }
...
async function generarPDFPedido(pool, pedidoId, empresaId) {
  ...
  doc.rect(0, 0, 595, 110).fill("#eff6ff");

  if (empresa.logo) {
    try {
      const cleanLogo = empresa.logo.replace(/^\/+/, "").replace("uploads/", "");
      const logoPath = path.join(__dirname, "public", "uploads", cleanLogo);
      if (fs.existsSync(logoPath)) {
        doc.image(logoPath, 40, 25, { fit: [90, 90], align: "left" });
      }
    } catch (e) {
  ...
  doc.fillColor(COLOR_PRIMARY).font("Helvetica-Bold").fontSize(24)
    .text(empresa.nombre || "EMPRESA", 150, 30);
  doc.fillColor(COLOR_SECONDARY).fontSize(16)
    .text("ORDEN DE COMPRA", 150, 60);
  ...
  doc.roundedRect(40, 130, 515, 80, 8).fillAndStroke("#ffffff", COLOR_BORDER);
  doc.fillColor(COLOR_TEXT).font("Helvetica-Bold").fontSize(11)
    .text("DATOS DEL PEDIDO", 55, 145);
  doc.font("Helvetica").fontSize(10)
    .text(`Pedido N°: ${pedido.id}`, 55, 170)
    .text(`Fecha: ${formatFechaPY(pedido.fecha_pedido)}`, 220, 170);

  doc.roundedRect(40, 230, 515, 110, 8).fillAndStroke("#ffffff", COLOR_BORDER);
  ...
  if (pedido.observacion) {
    doc.roundedRect(40, currentY, 515, 70, 8).fillAndStroke("#ffffff", COLOR_BORDER);
    doc.fillColor(COLOR_PRIMARY).font("Helvetica-Bold").fontSize(11)
      .text("OBSERVACIÓN", 55, currentY + 15);
    doc.fillColor(COLOR_TEXT).font("Helvetica").fontSize(10)
      .text(pedido.observacion, 55, currentY + 35, { width: 470 });
    currentY += 90;
  }

  currentY += 10;
  const startX = 40;
  const cols = [35, 180, 120, 60, 60, 80];
  const headers = ["#", "Producto", "Descripción", "Cant.", "Costo", "Total"];
  let x = startX;
  headers.forEach((h, i) => {
    doc.rect(x, currentY, cols[i], 25).fillAndStroke(COLOR_PRIMARY, COLOR_PRIMARY);
    doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(9)
      .text(h, x + 5, currentY + 8, { width: cols[i] - 10, align: "center" });
    x += cols[i];
  });
  currentY += 25;
  ...
  items.forEach((it, idx) => {
    ...
    values.forEach((val, i) => {
      doc.rect(xx, currentY, cols[i], 24)
        .fillAndStroke(idx % 2 === 0 ? "#ffffff" : COLOR_LIGHT, COLOR_BORDER);
      doc.fillColor(COLOR_TEXT).font("Helvetica").fontSize(9)
        .text(String(val), xx + 4, currentY + 7, {
          width: cols[i] - 8,
  ...
  currentY += 20;
  const totalBoxX = 340;
  doc.roundedRect(totalBoxX, currentY, 215, 80, 8).fillAndStroke("#ffffff", COLOR_BORDER);
  doc.fillColor(COLOR_TEXT).font("Helvetica").fontSize(10)
    .text("Subtotal:", totalBoxX + 15, currentY + 15)
    .text(fmtPY(pedido.subtotal || 0), totalBoxX + 100, currentY + 15, { width: 90, align: "right" })
    .text("IVA 10%:", totalBoxX + 15, currentY + 35)
    .text(fmtPY(pedido.iva || 0), totalBoxX + 100, currentY + 35, { width: 90, align: "right" });
  doc.font("Helvetica-Bold").fontSize(12).fillColor(COLOR_PRIMARY)
    .text("TOTAL:", totalBoxX + 15, currentY + 58)
    .text(fmtPY(pedido.total || 0), totalBoxX + 100, currentY + 58, { width: 90, align: "right" });

  doc.fillColor(COLOR_SECONDARY).fontSize(9).font("Helvetica")
    .text(`Documento generado automáticamente • ${empresa.nombre || "EMPRESA"}`, 40, 780, {
      align: "center",
      width: 515
    });
...
app.get("/api/pedidos/:id", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    if (!p) {
      return res.status(404).json({ error: "Pedido no encontrado o no pertenece a esta empresa" });
    }
    res.json(p);
  } catch (e) {
...
app.post("/api/pedidos", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    const provQ = await client.query(
      `SELECT id FROM proveedores WHERE id = $1 AND empresa_id = $2 LIMIT 1`,
      [proveedor_id, empresaId]
    );
    if (!provQ.rowCount) {
      await client.query("ROLLBACK");
      return res.status(404).json({ ok: false, msg: "Proveedor no encontrado o no pertenece a esta empresa" });
    }
    ...
    for (const it of items) {
      ...
      if (!productoId || cantidad <= 0 || precioUnit < 0) {
        await client.query("ROLLBACK");
        return res.status(400).json({ ok: false, msg: "Ítem inválido en el pedido" });
      }
      ...
      if (!prodQ.rowCount) {
        await client.query("ROLLBACK");
        return res.status(404).json({ ok: false, msg: `Producto ${productoId} no encontrado o no pertenece a esta empresa` });
      }
...
app.put("/api/pedidos/:id/estado", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    if (!prs.length) {
      await client.query("ROLLBACK");
      return res.status(404).json({ ok: false, msg: "Pedido no encontrado o no pertenece a esta empresa" });
    }
...
app.get("/api/formas-pago", requireEmpresa, async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ error: "Error cargando formas de pago" });
  }
});
...
app.post("/compras", requireEmpresa, async (req, res) => {
  ...
  if (!proveedor_id || !fecha || !Array.isArray(items) || !items.length) {
    return res.status(400).json({ ok: false, msg: "Proveedor, fecha e ítems son obligatorios" });
  }
  ...
  try {
    ...
    for (const it of items) {
      ...
      const cNew = sAnt + Number(it.cantidad) === 0
        ? Number(it.costo)
        : Math.round(((sAnt * cAnt) + (Number(it.cantidad) * Number(it.costo))) / (sAnt + Number(it.cantidad)));
      await client.query(
        `
        UPDATE productos
        SET stock = $1,
...
app.get("/compras/:id", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    if (!cab.rowCount) {
      return res.status(404).json({ ok: false, error: "Compra no encontrada o no pertenece a esta empresa" });
    }
    ...
    return res.json({ ok: true, ...cab.rows[0], items: items.rows });
  } catch (e) {
...
app.put("/compras/:id", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    if (!compraCheck.rowCount) {
      await client.query("ROLLBACK");
      return res.status(404).json({ ok: false, msg: "Compra no encontrada o no pertenece a esta empresa" });
    }
    ...
    const subtotalMonedaFinal = monedaFinal === "PYG"
      ? subtotalCalc
      : subtotalCalc / tipoCambioFinal;
    const ivaMonedaFinal = monedaFinal === "PYG"
      ? ivaCalc
      : ivaCalc / tipoCambioFinal;
    const totalMonedaFinal = monedaFinal === "PYG"
      ? totalCalc
...
app.post("/ventas", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    if (!formaPagoQ.rowCount) {
      return res.status(404).json({ ok: false, msg: "Forma de pago no encontrada o no pertenece a esta empresa" });
    }
    const formaPago = formaPagoQ.rows[0];
    const REQUIERE_COMPROBANTE = [
      "QR", "BNF", "Continental", "Banco Familiar", "Ueno Bank",
      "Banco Basa", "Mango", "Débito", "Crédito", "Otro Banco"
    ];
    const compStr = (nro_comprobante || "").toString().trim();
    if (REQUIERE_COMPROBANTE.includes(formaPago.nombre) && !compStr) {
      return res.status(400).json({ ok: false, msg: "Falta nro_comprobante" });
    }
    const tipoCajaNecesaria = formaPago.tipo === "efectivo" ? "efectivo" : "transferencia";
    await client.query("BEGIN");
    ...
    let clienteIdFinal = cliente_id && String(cliente_id) !== "0" ? Number(cliente_id) : null;
    if (clienteIdFinal) {
      ...
      if (!clienteQ.rowCount) {
        await client.query("ROLLBACK");
        return res.status(404).json({ ok: false, msg: "Cliente no encontrado o no pertenece a esta empresa" });
      }
    }
    const usuarioId = req.session?.user?.id || null;
    const cajeroNombre = req.session?.user?.nombre || req.session?.user?.usuario || "Sin usuario";
    const compFinal = REQUIERE_COMPROBANTE.includes(formaPago.nombre) ? compStr : null;
    const nombreFormaPago = String(formaPago.nombre || "").trim().toLowerCase();
    const esOtroBanco = nombreFormaPago.includes("otro banco");
    const bancoNombreFinal = esOtroBanco ? String(banco_nombre || "").trim() : null;
    console.log("=================================");
    ...
    for (const it of items) {
      ...
      if (!prodQ.rowCount) {
        await client.query("ROLLBACK");
        return res.status(404).json({ ok: false, msg: `Producto ID ${productoId} no encontrado o no pertenece a esta empresa` });
      }
      await client.query(
        `
      ...
      if (upd.rows.length === 0) {
        await client.query("ROLLBACK");
        return res.status(400).json({ ok: false, msg: `Stock insuficiente para el producto ID ${productoId}` });
      }
    }
...
app.get("/ventas/:id", requireEmpresa, async (req, res) => {
  try {
    ...
    if (ven.rows.length === 0) {
      return res.status(404).json({ ok: false, msg: "Venta no encontrada o no pertenece a esta empresa" });
    }
    const items = await pool.query(
      `
      SELECT
...
app.get("/ventas/:id/ticket", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    doc.font("Helvetica-Bold").fontSize(10)
       .text((venta.empresa_nombre || "MI EMPRESA").toUpperCase(), L, y, { width: W, align: "center" });
    y += 14;
    ...
    if (venta.empresa_ciudad && venta.empresa_telefono) {
      doc.text(`Tel: ${venta.empresa_telefono}    ${venta.empresa_ciudad.toUpperCase()}`, L, y, { width: W, align: "center" }); y += 11;
    ...
    for (const it of items.rows) {
      ...
      const subtotalTexto = moneda !== "PYG" ? subtotalItemEnMoneda(subtotal) : fmtGs(subtotal);
      doc.font("Helvetica").fontSize(7.5);
...
app.get("/compras/:id/pdf", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    doc.pipe(res);
    doc.fontSize(20).text(`Compras – ${comp.empresa_nombre || "Empresa"}`, { align: "center" });
    doc.moveDown(1);
    ...
    doc.moveDown(1);
    doc.fontSize(14).text(`TOTAL: Gs. ${Number(comp.total || 0).toLocaleString("es-PY")}`, { align: "right" });
    doc.end();
  } catch (err) {
...
app.delete("/compras/:id", requireEmpresa, async (req, res) => {
  ...
  try {
    ...
    if (!compraQ.rowCount) {
      await client.query("ROLLBACK");
      return res.json({ ok: false, msg: "Compra no encontrada o no pertenece a esta empresa" });
    }
    await client.query(
      `
      DELETE FROM compras_items
...
app.get("/caja/abierta", requireEmpresa, async (req, res) => {
  try {
    ...
    return res.status(500).json({ abierta: false, msg: "Error consultando caja" });
  }
});

// ================== CAJA - ESTADO (con filtro por fecha) ==================
...
app.get("/caja/resumen-dia", requireEmpresa, async (req, res) => {
  try {
    const empresaId = getEmpresaId(req);
    const diaParam = req.query.dia || req.query.fecha || hoyLocalPY();
...
app.get("/caja/resumen-mes", requireEmpresa, async (req, res) => {
  try {
    const empresaId = getEmpresaId(req);
    const mesParam = req.query.mes || req.query.fecha || hoyLocalPY();
...
app.get("/caja/resumen", requireEmpresa, async (req, res) => {
  try {
    const empresaId = getEmpresaId(req);
    const fechaParam = req.query.fecha || req.query.dia || req.query.mes || hoyLocalPY();
...
// APARTADO NUEVO: Movimientos con comprobante
  ...
  try {
    ...
    const itemsQ = await pool.query(
      `SELECT
         ...
         vi.precio,
         vi.subtotal,
         COALESCE(vi.iva_tipo, '10')   AS iva_tipo,
         p.nombre                      AS producto_nombre
       FROM ventas_items vi
    ...
    const r1y = cliY + 6;
    drawField("Fecha de Emisión:", `${dia}  /  ${mes}  /  ${anio}`, M + 6, r1y, 260);
    ...
    doc.font("Helvetica").fontSize(7.5)
       .text(`Contado [${esContado ? "X" : " "}]  Crédito [${esContado ? " " : "X"}]`, col2 + 82, r1y, { width: 130 });
    const r2y = r1y + lineH;
    ...
    const enLetras = typeof numToWords === "function"
      ...
      : "(ver importe arriba)";
    doc.font("Helvetica").fontSize(7.5)
       .text(`(En Letras): ${enLetras}`, tblX + 200, rowY + 6, { width: tblW - 370, align: "left" });
    const totBoxW = 160;
    const totBoxX = tblX + tblW - totBoxW;
    doc.rect(totBoxX, rowY, totBoxW, ftrH).stroke(COLOR_BORDE);
    doc.font("Helvetica-Bold").fontSize(10)
       .text(`Gs ${fmtGs(totalPyg)}`, totBoxX + 4, rowY + 8, { width: totBoxW - 8, align: "center" });
    rowY += ftrH;
    const ivaH = 20;
    lw(0.8);
    ...
    doc.text(`LIQUIDACIÓN DEL IVA (5%): Gs ${fmtGs(ivaCalc5)}`, tblX + 4, rowY + 5, { width: ivaColW - 8 });
    doc.moveTo(tblX + ivaColW, rowY).lineTo(tblX + ivaColW, rowY + ivaH).stroke(COLOR_BORDE);
    doc.text(`(10%): Gs ${fmtGs(ivaCalc10)}`, tblX + ivaColW + 4, rowY + 5, { width: ivaColW - 8, align: "center" });
    doc.moveTo(tblX + ivaColW * 2, rowY).lineTo(tblX + ivaColW * 2, rowY + ivaH).stroke(COLOR_BORDE);
    doc.text(`TOTAL IVA: Gs ${fmtGs(totalIva)}`, tblX + ivaColW * 2 + 4, rowY + 5, { width: ivaColW - 8, align: "right" });
    rowY += ivaH;
...
app.post("/caja/cerrar", requireEmpresa, async (req, res) => {
  try {
    ...
    const cajaIdFromBody = req.body?.caja_id ? Number(req.body.caja_id) : null;

    // Si se envía caja_id, usarlo directamente (para compatibilidad)
    if (cajaIdFromBody) {
      const check = await pool.query(
        `SELECT id FROM caja WHERE id = $1 AND empresa_id = $2 AND estado = 'abierta'`,
        [cajaIdFromBody, empresaId]
      );
      if (!check.rowCount) {
      ...
      const saldoFinal = Number(saldoCierre.rows[0]?.total || 0);
      await pool.query(
        `UPDATE caja SET estado = 'cerrada', cerrado_en = NOW(), saldo_cierre = $1
         WHERE id = $2 AND empresa_id = $3`,
        [saldoFinal, cajaIdFromBody, empresaId]
      );
      return res.json({ ok: true, caja_id: cajaIdFromBody, saldo_cierre: saldoFinal });
    }

    // Si no hay caja_id, buscar por tipo (sin filtrar por fecha)
    let query = `SELECT id FROM caja WHERE estado = 'abierta' AND empresa_id = $1`;
    const params = [empresaId];
    if (tipo) {
      params.push(tipo);
    ...
    return res.json({ ok: true, caja_id: cajaId, saldo_cierre: saldoFinal });
  } catch (err) {
  ...
  try {
    ...
    if (!empresaId) {
      const empresaExiste = await pool.query(
        `SELECT id FROM empresas WHERE nombre = 'SPYnet' ORDER BY id ASC LIMIT 1`
      );
    ...
    if (existe.rowCount) {
      await pool.query(
        `UPDATE usuarios SET empresa_id = COALESCE(empresa_id, $1) WHERE usuario = $2`,
        [empresaId, usuario]
      );
    ...
    await pool.query(
      ...
      VALUES ($1,$2,$3,$4,$5)
      `,
      [nombre, usuario, hash, "admin", empresaId]
    );
...
function drawSimpleTable(doc, {
  ...
  rows.forEach((row, idx) => {
    ...
    doc.strokeColor("#d1d5db").lineWidth(0.5)
      .rect(x, currentY, tableWidth, rowHeight).stroke();
    currentY += rowHeight;
  });
  doc.restore();
  return currentY;
...
app.post("/api/asistente-admin", requireEmpresa, async (req, res) => {
  try {
    ...
    const esTodo = !esHoy && !esAyer && !esMes;
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      return res.status(401).json({ ok: false, msg: "Usuario incorrecto" });
    }
    const u = rows[0];
    let ok = false;
    if (u.password_hash?.startsWith("$2")) {
    ...
    req.session.save(err => {
      if (err) {
        ...
        return res.status(500).json({ ok: false, msg: "Error guardando sesión" });
      }
...
app.post("/admin/login", async (req, res) => {
  ...
  try {
    ...
    req.session.save(err => {
      ...
      return res.json({ ok: true, user: req.session.admin });
    });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ ok: false, msg: "Error servidor" });
...
app.post("/api/empresas/logo", requireEmpresa, uploadLogo.single("logo"), async (req, res) => {
  try {
    const empresaId = getEmpresaId(req);
    if (!req.file) {
    ...
    console.error(e);
    res.status(500).json({ ok: false, msg: "No se pudo guardar logo" });
  }
});

app.post("/empresas/:id/logo", uploadLogo.single("logo"), async (req, res) => {
  try {
    const empresaId = req.params.id;
    const logoPath = `/uploads/logos/${req.file.filename}`;
    await pool.query(
